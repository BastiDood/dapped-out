    ...
    {:then contests}
        ...
        {:else}
            <div class="grid grid-cols-[repeat(auto-fill,minmax(30ch,1fr))]">
                {#each contests as { account: { host, slug, name, mint, stake, participants } }}
                    {@const total = stake.toNumber() * participants.length}
                    <a href="/contests/{host}/{slug}/" class="group card card-hover p-4">
                        <h4 class="h4">
                            <span class="group-hover:anchor group-hover:underline">{name}</span>
                        </h4>
